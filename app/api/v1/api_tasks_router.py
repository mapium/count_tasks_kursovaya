...
@router.get("/tasks", tags=["tasks"], response_model=TasksGroupedByDepartment, description="Вывод информации, сгруппированной по подразделениям")
...
@router.get("/tasks/my-department", tags=["tasks"], response_model=Page[Tasks], description="Вывод всех задач подразделения текущего пользователя")
...
@router.get("/tasks/my", tags=["tasks"], response_model=Page[Tasks], description="Вывод всех задач текущего залогиненного пользователя")
...
@router.post("/tasks", tags=["tasks"], response_model=Tasks, description="только с правами менеджера и выше")
...
@router.put("/tasks/{id}", tags=["tasks"], description="только с правами менеджера и выше")
...
@router.delete("/tasks/{id}", tags=["tasks"], description="только с правами менеджера и выше")
...
@router.patch("/tasks/{task_id}/status", tags=["tasks"], response_model=Tasks, description="""Изменение статуса задачи (доступно создателю, исполнителю, администратору или руководителю отдела)
...
@router.post("/tasks/{task_id}/comments", tags=["tasks"], response_model=TaskComments, description="Добавление комментария к задаче (доступно создателю, исполнителю, администратору или руководителю отдела)")
